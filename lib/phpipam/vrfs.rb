module Vrfs
  def all_vrfs(token)
    self.class.get("/#{app_name}/vrf/", headers: { "token" => token })
  end
  ...
  def create_vrf(name,rd,  description, sections, token)
    options = { query: {"name" => name, "rd" => rd,"description" => description, "sections" => sections}, headers: { "token" => token }}
